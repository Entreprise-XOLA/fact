    ...
    <div class="row">
      ...
      <div class="col">
        ...
        <div class="row" style="margin-top: 20px !important; padding-left: 300px; padding-right: 0px !important;">
          <span style="margin-top: 17px !important;margin-right: 5px !important;">Lomé, le</span> <q-input borderless v-model="dateajout" id="dateajout" @change="dateeffet()"></q-input>
        </div>
      </div>
    </div>
  </q-form>
  ...
  <div class="q-pa-md">
    ...
    >
      ...
      <template v-slot:top>
        ...
        <q-btn class="q-ml-sm" color="red" :disable="loading" label="Supprimer l'honoraire" @click="removeRowho(selected)"></q-btn>
        <q-space></q-space>
        <q-input borderless dense debounce="300" color="primary" v-model="filter">
          <template v-slot:append>
            <q-icon name="search"></q-icon>
 ...
 data () {
   ...
   mounted () {
    ...
    this.$axios.get('http://localhost/Gestion-Facture/index.php/Gestionfacture/info_facture?idfacturetemp='+this.idfacture)
        .then((response) => {
          ...
          this.dateajout = response.data.infos[0].dateajout
  ...
  methods: {
    ...
    dateeffet(){
      ...
      this.$axios.get('http://localhost/Gestion-Facture/index.php/Gestionfacture/modification_date?idfacturetemp='+this.idfacture+'&dateajout='+this.dateajout)
        .then((response) => {
          this.dateajout = response.data.infos.dateajout
          // this.graph = true
          console.log(this.dateajout)
        })
        .catch(() => {
